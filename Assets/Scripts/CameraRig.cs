...
public class CameraRig : MonoBehaviour
{
    ...
    private void CalculateAveragePosition()
    {
        ...
        foreach (Transform tankTrans in _playerManager.GetTankTransform())
        {
            // Skip non-active object(s)
            if (!tankTrans.gameObject.activeSelf)
                continue;

            sumPos += tankTrans.position;
            activeTanksCount++;
        }
    ...
    protected float GetRequiredSize()
    {
        ...
        foreach(Transform target in _playerManager.GetTankTransform())
        {
