...
public class Shell : MonoBehaviour
{
    ...
    //If owned by enemy set 0, owned by playerTank set 1.
    public int _owner = 0;

    public GlobalData globalData;

    public void set_owner(int owner_num)
    {
        _owner = owner_num;
    }
    ...
    protected void OnTriggerEnter(Collider other)
    {
        if(_owner == 1)
        {
            if (globalData.trainingMode && other.CompareTag("EnemyTank"))
            {
                //Debug.Log("This is your enemy!");
                globalData.enemyHit += 1;
            }

            else if (globalData.trainingMode && other.CompareTag("FriendTank"))
            {
                //Debug.Log("This is your friend!");
                globalData.friendHit += 1;
            }
            else if (globalData.trainingMode)
            {
                //Debug.Log("You've missed your shot!");
    ...
    public void Explosion()
    {
        ...
        foreach( Collider collider in tanksCollider)
        {
            ...
            WatertankHealth waterTank = collider.GetComponent<WatertankHealth>();
            if (waterTank != null) {
                waterTank.TakeDamage( CalculateDamage(waterTank.transform.position) );
            }
            // Apply effects to tanks
            PlayerTank ptank = collider.GetComponent<PlayerTank>();
            if (ptank != null) {
                ptank.TakeDamage( CalculateDamage(ptank.transform.position) );
            }
            EnemyTank etank = collider.GetComponent<EnemyTank>();
            if (etank != null) {
                etank.TakeDamage( CalculateDamage(etank.transform.position) );
            }
        }
